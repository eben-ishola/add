...
async function run() {
    ...
    console.log('Backfill others atlasAccount');
    ...
    console.log(`Looking up atlasAccount for ${userIdSet.size} user(s)`);
    const userIds = Array.from(userIdSet).map((id) => new mongoose_1.default.Types.ObjectId(id));
    const users = await UserModel.find({ _id: { $in: userIds } }, { _id: 1, atlasAccount: 1 }).lean();
    const accountMap = new Map();
    for (const u of users) {
        const account = String(u.atlasAccount ?? '').trim();
        if (account) {
            accountMap.set(String(u._id), account);
        }
    }
    console.log(`Resolved atlasAccount for ${accountMap.size} / ${userIdSet.size} user(s)`);
    let updatedDocs = 0;
    ...
    for (const doc of docs) {
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            const existing = String(entry.atlasAccount ?? '').trim();
            if (existing) {
                skippedAlreadySet++;
                continue;
            }
            ...
            if (account) {
                updates.push({ index: i, atlasAccount: account });
                docChanged = true;
            }
        }
        if (!docChanged)
        ...
        for (const u of updates) {
            setFields[`entries.${u.index}.atlasAccount`] = u.atlasAccount;
        }
...
//# sourceMappingURL=backfill-others-atlas-account.js.map