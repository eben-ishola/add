...
var InconvenienceAllowanceService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.InconvenienceAllowanceService = void 0;
...
const MONTHLY_BASE_DAYS = 30;
let InconvenienceAllowanceService = InconvenienceAllowanceService_1 = class InconvenienceAllowanceService {
    constructor(inconvenienceModel, inconvenienceSettingModel, departmentModel, roleModel, subsidiaryModel, inconvenienceWorkflowModel, inconvenienceApprovalModel, userModel, branchModel, noticeService, workflowNotifier) {
        this.inconvenienceModel = inconvenienceModel;
        this.inconvenienceSettingModel = inconvenienceSettingModel;
        this.departmentModel = departmentModel;
        this.roleModel = roleModel;
        this.subsidiaryModel = subsidiaryModel;
        this.inconvenienceWorkflowModel = inconvenienceWorkflowModel;
        this.inconvenienceApprovalModel = inconvenienceApprovalModel;
        this.userModel = userModel;
        this.branchModel = branchModel;
        this.noticeService = noticeService;
        this.workflowNotifier = workflowNotifier;
    }
    async buildBranchByLowerName() {
        ...
        for (const b of branches) {
            ...
            if (key) {
                map.set(key, { branchId: b._id, name: b.name ?? '', gl: b.gl ?? '' });
            }
        }
        return map;
    }
    async onModuleInit() {
        try {
            const col = this.inconvenienceSettingModel.collection;
            const indexes = await col.indexes();
            const stale = indexes.find((idx) => idx.name === 'entity_1_department_1_role_1_frequency_1');
            if (stale) {
                await col.dropIndex('entity_1_department_1_role_1_frequency_1');
                console.log('Dropped stale index: entity_1_department_1_role_1_frequency_1');
            }
        }
        catch {
        }
    ...
    resolvePayableDays(staff, year, month, generatedAt) {
        const periodStart = new Date(year, month - 1, 1);
        const periodEnd = new Date(year, month, 0, 23, 59, 59, 999);
        const startDate = this.toDate(staff?.startDate);
        const exitDate = this.toDate(staff?.exitDate);
        if (exitDate && exitDate.getTime() < generatedAt.getTime())
            return null;
        if (startDate && startDate.getTime() > periodEnd.getTime())
            return null;
        const effectiveStart = startDate && startDate.getTime() > periodStart.getTime() ? startDate : periodStart;
        const effectiveEnd = exitDate && exitDate.getTime() < periodEnd.getTime() ? exitDate : periodEnd;
        if (effectiveEnd.getTime() < effectiveStart.getTime())
            return 0;
        const dayMs = 24 * 60 * 60 * 1000;
        const spanned = Math.floor((this.startOfDay(effectiveEnd).getTime() - this.startOfDay(effectiveStart).getTime()) /
            dayMs) + 1;
        return Math.max(0, Math.min(spanned, MONTHLY_BASE_DAYS));
    }
    startOfDay(value) {
    ...
    normalizeFrequency(value, fallback) {
        ...
        if (normalized === 'month' || normalized === 'monthly')
            return 'monthly';
        throw new common_1.BadRequestException('Frequency must be monthly.');
    }
    normalizeText(value) {
        return String(value ?? '').trim();
    }
    normalizeInconvenienceLevel(value, required = false) {
        const normalized = this.normalizeText(value).toLowerCase();
        if (!normalized && required) {
            throw new common_1.BadRequestException('Inconvenience level is required.');
        }
        return normalized;
    }
    ...
    hasGlobalAccess(user) {
        ...
        for (const name of permissions) {
            if (InconvenienceAllowanceService_1.SUPER_ADMIN_ROLE_NAMES.has(name)) {
                return true;
            }
        }
        const additional = Array.isArray(user?.additionalRoles)
        ...
        return sources.some((roleLike) => this.extractRoleNames(roleLike).some((name) => InconvenienceAllowanceService_1.SUPER_ADMIN_ROLE_NAMES.has(name)));
    }
    userHasAnyPermission(user, permissions) {
        const names = this.extractPermissionNames(user);
        if (names.has('all'))
            return true;
        return permissions.some((permission) => names.has(permission.toLowerCase()));
    }
    workflowHasUserInRoles(workflow, user, roles) {
        const identifiers = this.getUserIdentifierSet(user);
        if (!identifiers.size)
            return false;
        const roleBuckets = {
            initiator: workflow?.initiatorIds,
            reviewer: workflow?.reviewerIds,
            approver: workflow?.approverIds,
            poster: workflow?.postingIds,
            auditViewer: workflow?.auditViewerIds,
        };
        const expandedRoles = roles.includes('member')
            ? ['initiator', 'reviewer', 'approver', 'poster', 'auditViewer']
            : roles;
        return expandedRoles.some((role) => this.listIncludesUserIdentifier(roleBuckets[role], identifiers));
    }
    async assertInconvenienceWorkflowAccess(user, entity, roles, permissions, message) {
        if (this.hasGlobalAccess(user) || this.userHasAnyPermission(user, permissions)) {
            return;
        }
        const entityId = this.normalizeObjectId(entity, 'Entity');
        const workflow = await this.inconvenienceWorkflowModel
            .findOne({ entity: entityId })
            .lean()
            .exec();
        if (workflow && this.workflowHasUserInRoles(workflow, user, roles)) {
            return;
        }
    ...
    resolveStageFromStatus(status) {
        ...
        if (normalized === InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_APPROVAL) {
            return 'APPROVER';
        }
        if (normalized === InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_POSTING) {
            return 'POSTING';
        }
        if (normalized === InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.APPROVED) {
            return 'POSTED';
        }
        return 'REVIEWER';
    }
    buildInconvenienceApprovalLink(approvalId) {
        const id = this.normalizeText(approvalId);
        return id ? `/payroll-inconvenience-approvals/${id}` : '/payroll-inconvenience-approvals';
    }
    normalizeApprovalCurrentStage(approval) {
        ...
        if (status === InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.REJECTED) {
        ...
        if (status === InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.APPROVED) {
    ...
    resolveBatchContext(payload, records) {
        const entityId = payload?.entity ?? records[0]?.entity;
        const year = this.toNumber(payload?.year ?? records[0]?.year);
        const month = this.toNumber(payload?.month ?? records[0]?.month);
        const weekOfMonth = this.toNumber(payload?.weekOfMonth ?? records[0]?.weekOfMonth);
        if (!entityId || !year || !month || !weekOfMonth) {
            throw new common_1.BadRequestException('Entity, year, month, and week are required.');
        }
        return {
            normalizedEntity: this.normalizeObjectId(entityId, 'Entity'),
            year,
    ...
    buildProrationRows(records, payload) {
        const baseDaysDefault = this.toNumber(payload?.baseDays ?? MONTHLY_BASE_DAYS, MONTHLY_BASE_DAYS);
        return records.map((record) => {
            ...
            const days = Math.max(0, Math.min(this.toNumber(record?.days ?? payload?.days ?? baseDays, baseDays), MONTHLY_BASE_DAYS));
    ...
    async list(filters, user) {
        await this.assertInconvenienceWorkflowAccess(user, filters.entity, ['member'], ['view inconvenience allowance', 'view payroll', 'view payroll settings'], 'You do not have permission to view inconvenience allowance.');
        const query = {};
        if (filters.entity) {
        ...
        const rows = await this.inconvenienceModel.find(query).lean().exec();
        return { status: 200, data: rows };
    }
    async listSettings(filters, user) {
        await this.assertInconvenienceWorkflowAccess(user, filters.entity, ['member'], ['view inconvenience allowance', 'view payroll', 'view payroll settings'], 'You do not have permission to view inconvenience allowance settings.');
        const query = {};
        if (filters.entity) {
            query.entity = this.normalizeObjectId(filters.entity, 'Entity');
        }
        if (filters.inconvenienceLevel) {
            const inconvenienceLevel = this.normalizeInconvenienceLevel(filters.inconvenienceLevel);
            query.inconvenienceLevel = new RegExp(`^${this.escapeRegex(inconvenienceLevel)}$`, 'i');
        }
        ...
        if (hasPaginationRequest) {
            ...
            const [rows, total] = await Promise.all([
                this.inconvenienceSettingModel
                    .find(query)
                    .sort({ createdAt: -1 })
                    .skip(skip)
                    .limit(limit)
                    .lean()
                    .exec(),
                this.inconvenienceSettingModel.countDocuments(query),
            ]);
            return {
                status: 200,
                data: rows,
        ...
        const rows = await this.inconvenienceSettingModel
            .find(query)
            .sort({ createdAt: -1 })
            .lean()
            .exec();
    ...
    async getWorkflowConfigs(entity, user) {
        const hasConfigPermission = this.hasGlobalAccess(user) ||
        ...
        if (entity) {
            await this.assertInconvenienceWorkflowAccess(user, entity, ['member'], ['view inconvenience allowance', 'view payroll', 'view payroll settings'], 'You do not have permission to view inconvenience workflow configuration.');
        }
        const query = {};
        if (entity) {
            ...
            const identifiers = this.getUserIdentifierVariants(user);
            if (!identifiers.length) {
                throw new common_1.ForbiddenException('You do not have permission to view inconvenience workflow configuration.');
            }
            query.$or = [
                { initiatorIds: { $in: identifiers } },
                { reviewerIds: { $in: identifiers } },
                { auditViewerIds: { $in: identifiers } },
                { approverIds: { $in: identifiers } },
                { postingIds: { $in: identifiers } },
            ];
        }
        const configs = await this.inconvenienceWorkflowModel
            .find(query)
    ...
    async saveWorkflowConfig(payload) {
        ...
        const financeIds = this.normalizeUserIdList(payload?.finance ?? payload?.financeIds);
        const inconvenienceGl = this.normalizeText(payload?.inconvenienceGl);
        ...
        const config = await this.inconvenienceWorkflowModel
    ...
    async upsertSetting(payload) {
        ...
        const inconvenienceLevel = this.normalizeInconvenienceLevel(payload?.inconvenienceLevel, true);
        const amount = this.normalizeAmount(payload?.amount);
        const filter = { entity: normalizedEntity, inconvenienceLevel };
        const update = { $set: { entity: normalizedEntity, inconvenienceLevel, amount } };
        try {
            const saved = await this.inconvenienceSettingModel
            ...
            if (error?.code === 11000) {
                const retried = await this.inconvenienceSettingModel
                ...
                const byRegex = await this.inconvenienceSettingModel
                    .findOneAndUpdate({ entity: normalizedEntity, inconvenienceLevel: new RegExp(`^${inconvenienceLevel}$`, 'i') }, update, { new: true })
                    .lean()
                    .exec();
                if (byRegex) {
                    return { status: 200, data: byRegex };
                }
                throw new common_1.BadRequestException(`A setting for inconvenience level "${inconvenienceLevel}" already exists for this entity.`);
            }
            throw new common_1.BadRequestException(error?.message ?? 'Unable to save inconvenience setting.');
        }
    }
    async bulkUpsertSettings(rows, options) {
        const records = Array.isArray(rows) ? rows : [];
        if (!records.length) {
            throw new common_1.BadRequestException('No inconvenience setting records provided.');
        }
        ...
        for (let index = 0; index < records.length; index += 1) {
            ...
            try {
                ...
                const inconvenienceLevelRaw = this.pickRowValue(row, [
                    'inconvenience_level',
                    'inconveniencelevel',
                    'inconvenience_tier',
                    'inconveniencetier',
                ...
                const inconvenienceLevel = this.normalizeInconvenienceLevel(inconvenienceLevelRaw, true);
                if (!inconvenienceLevel) {
                    throw new common_1.BadRequestException('Inconvenience level is required.');
                }
                const amountRaw = this.pickRowValue(row, [
                    'amount',
                    'inconvenience_amount',
        ...
        const result = await this.inconvenienceSettingModel.bulkWrite(operations, { ordered: false });
        return {
            status: 200,
    ...
    async deleteSetting(id) {
        ...
        const deleted = await this.inconvenienceSettingModel
            .findByIdAndDelete(new mongoose_2.Types.ObjectId(normalizedId))
            .lean()
            .exec();
        if (!deleted) {
            throw new common_1.NotFoundException('Inconvenience setting not found.');
        }
    ...
    async preview(payload, user) {
        const records = Array.isArray(payload?.records)
            ...
            : Array.isArray(payload)
                ...
                : [];
        if (!records.length) {
            throw new common_1.BadRequestException('No inconvenience allowance records provided.');
        }
        const context = this.resolveContext(payload, records);
        await this.assertInconvenienceWorkflowAccess(user, context.normalizedEntity, ['initiator'], ['view inconvenience allowance', 'view payroll', 'view payroll settings'], 'You are not allowed to initiate inconvenience allowance for this entity.');
    ...
    async generate(payload, user) {
        const entity = this.normalizeObjectId(payload?.entity, 'Entity');
        await this.assertInconvenienceWorkflowAccess(user, entity, ['initiator'], ['view inconvenience allowance', 'view payroll', 'view payroll settings'], 'You are not allowed to initiate inconvenience allowance for this entity.');
        ...
        const frequency = this.normalizeFrequency(payload?.frequency, 'monthly');
        ...
        const settings = await this.inconvenienceSettingModel.find(settingsQuery).lean().exec();
        if (!settings.length) {
            return { status: 200, data: [], totals: { total: 0 } };
        }
        const amountLookup = new Map();
        const inconvenienceLevels = new Set();
        settings.forEach((setting) => {
            const inconvenienceLevel = this.normalizeInconvenienceLevel(setting?.inconvenienceLevel);
            const amount = Number(setting?.amount ?? 0);
            if (!inconvenienceLevel || !Number.isFinite(amount))
                return;
            amountLookup.set(inconvenienceLevel, amount);
            inconvenienceLevels.add(inconvenienceLevel);
        });
        if (!amountLookup.size) {
            return { status: 200, data: [], totals: { total: 0 } };
        }
        const inconvenienceLevelMatchers = Array.from(inconvenienceLevels).map((level) => new RegExp(`^${this.escapeRegex(level)}$`, 'i'));
        const staffQuery = {
            entity,
            status: { $regex: /^\s*(?:active|approved)\s*$/i },
            inconvenienceLevel: { $in: inconvenienceLevelMatchers },
        };
        ...
        const staffList = await this.userModel
            .find(staffQuery)
            .select('_id firstName lastName middleName staffId addosserAccount department inconvenienceLevel startDate exitDate')
            .lean()
            .exec();
        if (!staffList.length) {
            return { status: 200, data: [], totals: { total: 0 } };
        }
        const existingProrations = await this.inconvenienceModel
            .find({
            entity,
            year,
            month,
        ...
        const rows = staffList
            ...
            const inconvenienceLevel = this.normalizeInconvenienceLevel(staff?.inconvenienceLevel);
            if (!staffObjectId || !departmentId || !inconvenienceLevel)
                return null;
            const baseAmount = amountLookup.get(inconvenienceLevel);
            if (baseAmount === undefined)
                return null;
            const existing = existingMap.get(staffObjectId);
            const baseDays = this.toNumber(existing?.baseDays, MONTHLY_BASE_DAYS);
            const payableDays = this.resolvePayableDays(staff, year, month, generatedAt);
            if (payableDays === null)
                return null;
            const days = existing?.days === undefined || existing?.days === null
                ? payableDays
                : Math.max(0, Math.min(this.toNumber(existing.days, payableDays), MONTHLY_BASE_DAYS));
            const proratedAmount = baseDays > 0 ? Number(((baseAmount / baseDays) * days).toFixed(2)) : 0;
    ...
    async upsert(payload, user) {
        ...
        const staffIds = rows.map((r) => r.staff).filter(Boolean);
        ...
        for (const u of staffUsers) {
            ...
            if (typeof branch === 'string' && branch.trim()) {
                if (!branchByLowerName) {
                    branchByLowerName = await this.buildBranchByLowerName();
                }
                const hit = branchByLowerName.get(branch.trim().toLowerCase());
                if (hit) {
                    branchByStaff.set(String(u._id), { ...hit });
                }
            }
        }
                ...
                updateOne: {
                    ...
                    update: {
                        $set: {
                            ...
                            branch: branchInfo?.branchId ?? null,
        ...
        const result = await this.inconvenienceModel.bulkWrite(ops, { ordered: false });
        return {
            status: 200,
            updated: result.modifiedCount ?? 0,
            upserted: result.upsertedCount ?? 0,
    ...
    async submitForReview(payload, actor) {
        ...
        const context = this.resolveBatchContext(payload, records);
        await this.assertInconvenienceWorkflowAccess(actor, context.normalizedEntity, ['initiator'], ['view inconvenience allowance', 'view payroll', 'view payroll settings'], 'You are not allowed to initiate inconvenience allowance for this entity.');
        ...
        const staffUsers = staffIds.length
            ? await this.userModel
                ...
                .select('_id branch department addosserAccount')
                .lean()
                .exec()
            : [];
        const branchByStaff = new Map();
        const departmentByStaff = new Map();
        const addosserAccountByStaff = new Map();
        let branchByLowerName = null;
        for (const u of staffUsers) {
            const branch = u?.branch;
            if (branch && typeof branch === 'object') {
                ...
                const hit = branchByLowerName.get(branch.trim().toLowerCase());
                if (hit) {
                    branchByStaff.set(String(u._id), { name: hit.name, gl: hit.gl });
                }
            }
            const dept = u?.department;
            if (dept) {
                departmentByStaff.set(String(u._id), String(dept));
            }
            const addosserAccount = this.normalizeText(u?.addosserAccount);
            if (addosserAccount) {
                addosserAccountByStaff.set(String(u._id), addosserAccount);
            }
        }
        const departmentByRecord = new Map();
        for (const rec of records) {
            const staffKey = String(rec?.staff ?? rec?.staffId ?? rec?._id ?? rec?.id ?? '');
            const deptId = this.normalizeText(rec?.department ?? rec?.departmentId);
            if (staffKey && deptId) {
                departmentByRecord.set(staffKey, deptId);
            }
        }
        const allDeptIds = Array.from(new Set([...departmentByRecord.values(), ...departmentByStaff.values()].filter((id) => id && mongoose_2.Types.ObjectId.isValid(id))));
        const departmentDocs = allDeptIds.length
            ? await this.departmentModel
                .find({ _id: { $in: allDeptIds.map((id) => new mongoose_2.Types.ObjectId(id)) } })
                .select('_id name')
                .lean()
                .exec()
            : [];
        ...
        for (const dept of departmentDocs) {
            if (dept?._id) {
                departmentNameById.set(String(dept._id), String(dept.name ?? ''));
            }
        }
        const data = prorationRows.map((row) => {
            const staffKey = String(row.staff);
            const branchInfo = branchByStaff.get(staffKey);
            const rowDepartment = departmentByRecord.get(staffKey) ??
                departmentByStaff.get(staffKey) ??
                '';
            const recordAddosserAccount = this.normalizeText(row.addosserAccount);
            const resolvedAddosserAccount = recordAddosserAccount && recordAddosserAccount !== '-'
                ? recordAddosserAccount
                : addosserAccountByStaff.get(staffKey) ?? '';
            return {
                ...row,
                staff: staffKey,
                addosserAccount: resolvedAddosserAccount,
                branch: branchInfo?.name ?? '',
                branchGL: branchInfo?.gl ?? '',
                entity: String(context.normalizedEntity),
                department: rowDepartment,
                departmentName: rowDepartment ? departmentNameById.get(rowDepartment) ?? '' : '',
                year: context.year,
                month: context.month,
                weekOfMonth: context.weekOfMonth,
            };
        ...
        const workflow = await this.inconvenienceWorkflowModel
            ...
            .exec();
        if (!workflow) {
            throw new common_1.BadRequestException('Inconvenience workflow is not configured for this entity.');
        }
        ...
        const pendingStatuses = [
            InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_REVIEW,
            InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_APPROVAL,
            InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_POSTING,
        ];
        const existing = await this.inconvenienceApprovalModel
            .findOne({
            entity: context.normalizedEntity,
        ...
        if (existing) {
            ...
            existing.status = InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_REVIEW;
            ...
            await this.workflowNotifier?.dispatch({
                module: 'inconvenience_allowance',
                event: 'submitted',
                doc: existing,
            });
            return {
                status: 200,
                message: 'Inconvenience allowance sent to reviewer.',
                approvalId: existing._id,
            };
        }
        const approval = await this.inconvenienceApprovalModel.create({
            entity: context.normalizedEntity,
            year: context.year,
            month: context.month,
            weekOfMonth: context.weekOfMonth,
            frequency,
            status: InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_REVIEW,
    ...
    async getApprovals(user, options) {
        ...
        if (requestedAssignedId &&
            ...
            !hasGlobalAccess) {
            throw new common_1.ForbiddenException('You are not allowed to filter inconvenience approvals for another user.');
        }
        ...
        const rows = await this.inconvenienceApprovalModel
            .find(query)
    ...
    async getApprovalById(id, user) {
        const approval = await this.inconvenienceApprovalModel
            .findById(id)
            .populate('entity', 'name short code gl')
            .populate('department', 'name')
            .lean()
            .exec();
        if (!approval) {
            throw new common_1.NotFoundException('Inconvenience approval not found.');
        }
        if (!this.canViewApproval(user, approval)) {
            throw new common_1.BadRequestException('You are not allowed to view this approval.');
        }
        await this.backfillApprovalAddosserAccounts(id, approval);
    ...
    async backfillApprovalAddosserAccounts(id, approval) {
        const rows = Array.isArray(approval?.data) ? approval.data : [];
        if (!rows.length)
            return;
        const missingStaffIds = new Set();
        rows.forEach((row) => {
            const current = this.normalizeText(row?.addosserAccount);
            if (current && current !== '-')
                return;
            const staffId = this.normalizeText(row?.staff);
            if (staffId && mongoose_2.Types.ObjectId.isValid(staffId)) {
                missingStaffIds.add(staffId);
            }
        });
        if (!missingStaffIds.size)
            return;
        const users = await this.userModel
            .find({ _id: { $in: Array.from(missingStaffIds).map((sid) => new mongoose_2.Types.ObjectId(sid)) } })
            .select('_id addosserAccount')
            .lean()
            .exec();
        const accountByStaff = new Map();
        for (const u of users) {
            const account = this.normalizeText(u?.addosserAccount);
            if (account)
                accountByStaff.set(String(u._id), account);
        }
        if (!accountByStaff.size)
            return;
        let mutated = false;
        const nextRows = rows.map((row) => {
            ...
            if (current && current !== '-')
                return row;
            const staffId = this.normalizeText(row?.staff);
            const resolved = staffId ? accountByStaff.get(staffId) : undefined;
            if (!resolved)
                return row;
            mutated = true;
            return { ...row, addosserAccount: resolved };
        });
        if (!mutated)
            return;
        approval.data = nextRows;
        await this.inconvenienceApprovalModel.updateOne({ _id: id }, { $set: { data: nextRows } }).exec();
    }
    assertFinanceCommenter(user) {
        if ((0, access_control_util_2.userDepartmentIncludes)(user, 'audit')) {
            throw new common_1.ForbiddenException('Audit department has read-only access.');
        }
        if ((0, access_control_util_2.userIsSuperAdmin)(user))
            return;
    ...
    async updateFinanceComment(id, user, comment) {
        const approval = await this.inconvenienceApprovalModel.findById(id).exec();
        if (!approval) {
            throw new common_1.NotFoundException('Inconvenience approval not found.');
        }
        this.assertFinanceCommenter(user);
        const financeComment = this.normalizeText(comment);
        approval.financeComment = financeComment || undefined;
        const actorId = this.normalizeUserId(user?._id ?? user?.id ?? user?.userId);
        approval.financeCommentBy = actorId ? new mongoose_2.Types.ObjectId(actorId) : undefined;
    ...
    async approveApproval(id, user, comment) {
        ...
        if (approval.status === InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.REJECTED) {
            throw new common_1.BadRequestException('This approval has already been rejected.');
        }
        if (approval.status === InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.APPROVED) {
            throw new common_1.BadRequestException('This approval has already been completed.');
        }
        if (approval.status === InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_REVIEW) {
            ...
            approval.status = InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_APPROVAL;
            ...
            return { status: 200, message: 'Inconvenience batch moved to approvers.' };
        }
        ...
        approval.status = InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_POSTING;
        ...
        return { status: 200, message: 'Inconvenience batch approved and forwarded for posting.' };
    }
    async rejectApproval(id, user, reason) {
        ...
        else if (approval.status === InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_APPROVAL) {
            if (!this.canApproveApproval(user, approval)) {
                throw new common_1.BadRequestException('Only configured approvers can reject at this stage.');
            }
        }
        else if (approval.status === InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_POSTING) {
        ...
        approval.status = InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.REJECTED;
        ...
        return { status: 200, message: 'Inconvenience batch rejected.' };
    }
    async markPostingComplete(id, user) {
        ...
        if (approval.status !== InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.PENDING_POSTING) {
        ...
        approval.status = InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.APPROVED;
        ...
        return { status: 200, message: 'Inconvenience batch marked as posted.' };
    }
    async switchApprovalAccount(_user, approvalId, staffId, accountType) {
        ...
        const approval = await this.inconvenienceApprovalModel.findById(approvalId).exec();
        if (!approval) {
            throw new common_1.NotFoundException('Inconvenience approval not found.');
        }
        if (String(approval.status ?? '').toUpperCase() ===
            InconvenienceAllowanceService_1.INCONVENIENCE_APPROVAL_STATUS.APPROVED) {
        ...
        const idsOf = (row) => [row?.staff, row?.staffId, row?.staffObjectId, row?.id]
        ...
        const userLookupIds = Array.from(new Set(matches
            .map((row) => this.normalizeUserId(row?.staff))
            .filter((value) => Boolean(value))));
        let userDoc = userLookupIds.length
            ? await this.userModel
                .findOne({ _id: { $in: userLookupIds } })
    ...
    async switchApprovalAccountForAll(_user, approvalId, accountType) {
        ...
        const objectIds = Array.from(new Set(rows
            .map((row) => this.normalizeUserId(row?.staff))
            .filter((value) => Boolean(value))));
        const usersById = new Map(objectIds.length
            ? (await this.userModel
                .find({ _id: { $in: objectIds } })
                .select('_id staffId atlasAccount addosserAccount')
                .lean()
                .exec()).map((user) => [String(user._id), user])
            : []);
        const fallbackCodes = Array.from(new Set(rows
            .filter((row) => !this.normalizeUserId(row?.staff))
            .map((row) => String(row?.staffId ?? '').trim())
            .filter(Boolean)));
        const usersByCode = new Map(fallbackCodes.length
            ? (await this.userModel
                .find({ staffId: { $in: fallbackCodes } })
                .select('_id staffId atlasAccount addosserAccount')
                .lean()
                .exec()).map((user) => [String(user.staffId ?? '').trim().toLowerCase(), user])
            : []);
        let switched = 0;
        let skipped = 0;
        for (const row of rows) {
            const userDoc = usersById.get(this.normalizeUserId(row?.staff) ?? '') ??
                usersByCode.get(String(row?.staffId ?? '').trim().toLowerCase());
            const source = normalizedType === 'atlas'
                ? userDoc?.atlasAccount
                : userDoc?.addosserAccount;
            const account = source === null || source === undefined ? '' : String(source).trim();
    ...
    async getWorkflowRole(user, options) {
        ...
        const configs = await this.inconvenienceWorkflowModel.find(query).lean().exec();
        ...
        for (const config of configs) {
            ...
            const inFinance = this.listIncludesUserIdentifier(config.financeIds, userIdentifiers);
...
exports.InconvenienceAllowanceService = InconvenienceAllowanceService;
InconvenienceAllowanceService.INCONVENIENCE_APPROVAL_STATUS = {
...
InconvenienceAllowanceService.SUPER_ADMIN_ROLE_NAMES = access_control_util_1.SUPER_ADMIN_ROLE_NAME_SET_WITH_MD;
exports.InconvenienceAllowanceService = InconvenienceAllowanceService = InconvenienceAllowanceService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(inconvenience_allowance_schema_1.InconvenienceAllowance.name)),
    __param(1, (0, mongoose_1.InjectModel)(inconvenience_allowance_setting_schema_1.InconvenienceAllowanceSetting.name)),
    ...
    __param(5, (0, mongoose_1.InjectModel)(inconvenience_workflow_schema_1.InconvenienceWorkflowConfig.name)),
    __param(6, (0, mongoose_1.InjectModel)(inconvenience_allowance_approval_schema_1.InconvenienceAllowanceApproval.name)),
    __param(7, (0, mongoose_1.InjectModel)(user_schema_1.User.name)),
    __param(8, (0, mongoose_1.InjectModel)(branch_schema_1.Branch.name)),
...
], InconvenienceAllowanceService);
//# sourceMappingURL=inconvenience-allowance.service.js.map